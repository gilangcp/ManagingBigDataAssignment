...
class CountKeywordByUniqueUserPerMonthTest extends FlatSpec with Matchers with BeforeAndAfter {
  ...
  it should "count words in a text" in {
    ...
    // calculate output
    val output: RDD[(String, Int)] = CountKeywordByUniqueUserPerMonth.doJob(input)
    // check output
